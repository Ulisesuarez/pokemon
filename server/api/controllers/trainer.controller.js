...
exports.findById = function (req, res, next) {
  Trainer.findById(req.params.id).populate('pokemons._id').exec(function (err, trainer) {
  ...
  Trainer.findById(req.params.id, function (err, trainer) {
    ...
    let pokemon = {
      ...
      if(trainer.pokemons.length >= 6){
        return next(new Error("Can't add new pokemons, you already have the maximum (6)"))
      }
...
exports.deletePokemon = function (req, res, next) {
  Trainer.findById(req.params.id, function (err, trainer) {
    if (err) return next(err);
    trainer.pokemon = trainer.pokemons
      .filter(pokemon=>pokemon._instanceID !==rq.params.pokemon)
    trainer.save()
    res.send(trainer);
  });
};
